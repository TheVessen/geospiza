...
public static class ComputeServer
{
    ...
    private static WebResponse DoPost(string function, string json)
    {
...
public static class PythonCompute
{
    ...
    {
        return ComputeServer.PostWithConverter<ArchivableDictionary>(ApiAddress(), new ArchivableDictionaryResolver(),
            script, input);
    }
...
public static class BezierCurveCompute
{
    ...
    /// </param>
    /// <returns>A new array of bezier curves. The array can be empty and might contain null items.</returns>
    public static BezierCurve[] CreateCubicBeziers(Remote<Curve> sourceCurve, double distanceTolerance,
...
public static class BrepCompute
{
    ...
    ///     The parameter direction (0 = U, 1 = V). The face's underlying surface must be closed in this
    ///     direction.
    /// </param>
    ...
    /// <param name="density">The number of rulings across the surface.</param>
    /// <returns>The output Breps if successful, otherwise an empty array.</returns>
    public static Brep[] CreateDevelopableLoft(Remote<Curve> crv0, Remote<Curve> crv1, bool reverse0, bool reverse1,
        int density)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), crv0, crv1, reverse0, reverse1, density);
    }
    ...
    /// </param>
    /// <returns>The output Breps if successful, otherwise an empty array.</returns>
    public static Brep[] CreateDevelopableLoft(Remote<NurbsCurve> rail0, Remote<NurbsCurve> rail1,
    ...
    /// </param>
    /// <returns>A boundary representation, or null on error.</returns>
    public static Brep CreateFromCornerPoints(Point3d corner1, Point3d corner2, Point3d corner3, Point3d corner4,
        double tolerance)
    {
    ...
    /// </returns>
    public static Brep CreateFromOffsetFace(BrepFace face, double offsetDistance, double offsetTolerance,
        bool bothSides, bool createSolid)
    {
    ...
    ///     Edge must be within this angle tolerance in order for contiguous edges to be
    ...
    /// </param>
    /// <returns>The merged surfaces as a Brep if successful, null if not successful.</returns>
    public static Brep MergeSurfaces(Remote<Surface> surface0, Remote<Surface> surface1, double tolerance,
    ...
    /// </param>
    /// <returns>The merged Brep if successful, null if not successful.</returns>
    public static Brep MergeSurfaces(Remote<Brep> brep0, Remote<Brep> brep1, double tolerance,
        double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), brep0, brep1, tolerance, angleToleranceRadians);
    }
    ...
    /// <param name="smooth">
    ///     The surface will be smooth. This makes the surface behave better for control point editing, but
    ///     may alter the shape of both surfaces.
    /// </param>
    /// <returns>The merged Brep if successful, null if not successful.</returns>
    public static Brep MergeSurfaces(Brep brep0, Brep brep1, double tolerance, double angleToleranceRadians,
        Point2d point0, Point2d point1, double roundness, bool smooth)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), brep0, brep1, tolerance, angleToleranceRadians, point0, point1,
            roundness, smooth);
    }
    ...
    /// <returns>The merged Brep if successful, null if not successful.</returns>
    public static Brep MergeSurfaces(Remote<Brep> brep0, Remote<Brep> brep1, double tolerance,
        double angleToleranceRadians, Point2d point0, Point2d point1, double roundness, bool smooth)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), brep0, brep1, tolerance, angleToleranceRadians, point0, point1,
            roundness, smooth);
    }
    ...
    ///     Brep fit through input on success, or null on error.
    /// </returns>
    public static Brep CreatePatch(Remote<IEnumerable<GeometryBase>> geometry, Remote<Surface> startingSurface,
        double tolerance)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), geometry, startingSurface, tolerance);
    }
    ...
    ///     A brep fit through input on success, or null on error.
    /// </returns>
    public static Brep CreatePatch(IEnumerable<GeometryBase> geometry, Surface startingSurface, int uSpans, int vSpans,
        bool trim, bool tangency, double pointSpacing, double flexibility, double surfacePull, bool[] fixEdges,
        double tolerance)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), geometry, startingSurface, uSpans, vSpans, trim, tangency,
            pointSpacing, flexibility, surfacePull, fixEdges, tolerance);
    }
    ...
    /// </returns>
    public static Brep CreatePatch(Remote<IEnumerable<GeometryBase>> geometry, Remote<Surface> startingSurface,
        int uSpans, int vSpans, bool trim, bool tangency, double pointSpacing, double flexibility, double surfacePull,
        bool[] fixEdges, double tolerance)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), geometry, startingSurface, uSpans, vSpans, trim, tangency,
            pointSpacing, flexibility, surfacePull, fixEdges, tolerance);
    }
    ...
    ///     If False, Global (radius is linearly blended from one end to the other, creating pipes that taper from one radius
    ///     to the other) is applied.
    /// </param>
    ...
    public static Brep[] CreatePipe(Curve rail, double radius, bool localBlending, PipeCapMode cap, bool fitRail,
        double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, radius, localBlending, cap, fitRail, absoluteTolerance,
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreatePipe(Remote<Curve> rail, double radius, bool localBlending, PipeCapMode cap,
        bool fitRail, double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, radius, localBlending, cap, fitRail, absoluteTolerance,
            angleToleranceRadians);
    }
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreatePipe(Curve rail, IEnumerable<double> railRadiiParameters, IEnumerable<double> radii,
        bool localBlending, PipeCapMode cap, bool fitRail, double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, railRadiiParameters, radii, localBlending, cap, fitRail,
            absoluteTolerance, angleToleranceRadians);
    }
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreatePipe(Remote<Curve> rail, IEnumerable<double> railRadiiParameters,
        IEnumerable<double> radii, bool localBlending, PipeCapMode cap, bool fitRail, double absoluteTolerance,
        double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, railRadiiParameters, radii, localBlending, cap, fitRail,
            absoluteTolerance, angleToleranceRadians);
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreateThickPipe(Curve rail, double radius0, double radius1, bool localBlending,
        PipeCapMode cap, bool fitRail, double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, radius0, radius1, localBlending, cap, fitRail,
            absoluteTolerance, angleToleranceRadians);
    }
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreateThickPipe(Remote<Curve> rail, double radius0, double radius1, bool localBlending,
        PipeCapMode cap, bool fitRail, double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, radius0, radius1, localBlending, cap, fitRail,
            absoluteTolerance, angleToleranceRadians);
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreateThickPipe(Curve rail, IEnumerable<double> railRadiiParameters,
        IEnumerable<double> radii0, IEnumerable<double> radii1, bool localBlending, PipeCapMode cap, bool fitRail,
        double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, railRadiiParameters, radii0, radii1, localBlending, cap,
            fitRail, absoluteTolerance, angleToleranceRadians);
    }
    ...
    /// </param>
    /// <returns>Array of Breps success.</returns>
    public static Brep[] CreateThickPipe(Remote<Curve> rail, IEnumerable<double> railRadiiParameters,
        IEnumerable<double> radii0, IEnumerable<double> radii1, bool localBlending, PipeCapMode cap, bool fitRail,
        double absoluteTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, railRadiiParameters, radii0, radii1, localBlending, cap,
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Remote<Curve> rail, Remote<IEnumerable<Curve>> shapes, bool closed,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, shapes, closed, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweepSegmented(Remote<Curve> rail, Remote<Curve> shape, bool closed,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, shape, closed, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails.</param>
    /// <returns>Array of Brep sweep results.</returns>
    public static Brep[] CreateFromSweepSegmented(Remote<Curve> rail, Remote<IEnumerable<Curve>> shapes, bool closed,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail, shapes, closed, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Remote<Curve> rail1, Remote<Curve> rail2, Remote<Curve> shape, bool closed,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail1, rail2, shape, closed, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Curve rail1, Curve rail2, IEnumerable<Curve> shapes, bool closed,
        double tolerance)
    {
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Remote<Curve> rail1, Remote<Curve> rail2, Remote<IEnumerable<Curve>> shapes,
    ...
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Curve rail1, Curve rail2, IEnumerable<Curve> shapes, Point3d start,
        Point3d end, bool closed, double tolerance, SweepRebuild rebuild, int rebuildPointCount, double refitTolerance,
        bool preserveHeight)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail1, rail2, shapes, start, end, closed, tolerance, rebuild,
            rebuildPointCount, refitTolerance, preserveHeight);
    }
    ...
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweep(Remote<Curve> rail1, Remote<Curve> rail2, Remote<IEnumerable<Curve>> shapes,
        Point3d start, Point3d end, bool closed, double tolerance, SweepRebuild rebuild, int rebuildPointCount,
        double refitTolerance, bool preserveHeight)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail1, rail2, shapes, start, end, closed, tolerance, rebuild,
            rebuildPointCount, refitTolerance, preserveHeight);
    }
    ...
    /// <summary>
    ///     Makes a 2 rail sweep. Like CreateFromSweep but the result is split where parameterization along a rail changes
    ///     abruptly.
    /// </summary>
    /// <param name="rail1">Rail to sweep shapes along</param>
    /// <param name="rail2">Rail to sweep shapes along</param>
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweepInParts(Curve rail1, Curve rail2, IEnumerable<Curve> shapes,
        IEnumerable<Point2d> rail_params, bool closed, double tolerance)
    {
    ...
    /// <param name="tolerance">Tolerance for fitting surface and rails</param>
    /// <returns>Array of Brep sweep results</returns>
    public static Brep[] CreateFromSweepInParts(Remote<Curve> rail1, Remote<Curve> rail2,
        Remote<IEnumerable<Curve>> shapes, IEnumerable<Point2d> rail_params, bool closed, double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), rail1, rail2, shapes, rail_params, closed, tolerance);
    }
    ...
    public static Brep[] CreateFromTaperedExtrude(Curve curveToExtrude, double distance, Vector3d direction,
        Point3d basePoint, double draftAngleRadians, ExtrudeCornerType cornerType, double tolerance,
        double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curveToExtrude, distance, direction, basePoint,
            draftAngleRadians,
            cornerType, tolerance, angleToleranceRadians);
    }
    ...
    /// <param name="angleToleranceRadians">angle tolerance to use for the extrusion</param>
    /// <returns>array of breps on success</returns>
    public static Brep[] CreateFromTaperedExtrude(Remote<Curve> curveToExtrude, double distance, Vector3d direction,
        Point3d basePoint, double draftAngleRadians, ExtrudeCornerType cornerType, double tolerance,
        double angleToleranceRadians)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curveToExtrude, distance, direction, basePoint,
    ...
    public static Brep[] CreateFromTaperedExtrude(Curve curveToExtrude, double distance, Vector3d direction,
        Point3d basePoint, double draftAngleRadians, ExtrudeCornerType cornerType)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curveToExtrude, distance, direction, basePoint,
            draftAngleRadians,
            cornerType);
    ...
    public static Brep[] CreateFromTaperedExtrudeWithRef(Curve curve, Vector3d direction, double distance,
        double draftAngle, Plane plane, double tolerance)
    {
    ...
    /// <param name="tolerance">The intersecting and trimming tolerance.</param>
    /// <returns>An array of Breps if successful.</returns>
    public static Brep[] CreateFromTaperedExtrudeWithRef(Remote<Curve> curve, Vector3d direction, double distance,
        double draftAngle, Plane plane, double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curve, direction, distance, draftAngle, plane, tolerance);
    }
    ...
    /// <param name="rev0">
    ///     If false, edge0 will be used in its natural direction. If true, edge0 will be used in the reversed
    ///     direction.
    /// </param>
    ...
    /// <param name="rev1">
    ///     If false, edge1 will be used in its natural direction. If true, edge1 will be used in the reversed
    ///     direction.
    /// </param>
    /// <param name="continuity1">Continuity for the blend at the end.</param>
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateBlendSurface(BrepFace face0, BrepEdge edge0, Interval domain0, bool rev0,
        BlendContinuity continuity0, BrepFace face1, BrepEdge edge1, Interval domain1, bool rev1,
        BlendContinuity continuity1)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), face0, edge0, domain0, rev0, continuity0, face1, edge1, domain1,
            rev1, continuity1);
    }

    /// <summary>
    ///     Makes a curve blend between points on two surface edges. The blend will be tangent to the surfaces and
    ...
    public static Curve CreateBlendShape(BrepFace face0, BrepEdge edge0, double t0, bool rev0,
        BlendContinuity continuity0, BrepFace face1, BrepEdge edge1, double t1, bool rev1, BlendContinuity continuity1)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), face0, edge0, t0, rev0, continuity0, face1, edge1, t1, rev1,
            continuity1);
    }
    ...
    /// <param name="extend">
    ///     If true, then when one input surface is longer than the other, the fillet surface is extended to
    ///     the input surface edges.
    /// </param>
    /// <param name="tolerance">The tolerance. When in doubt, use the document's model absolute tolerance.</param>
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateFilletSurface(BrepFace face0, Point2d uv0, BrepFace face1, Point2d uv1, double radius,
        bool extend, double tolerance)
    {
    ...
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateFilletSurface(BrepFace face0, Point2d uv0, BrepFace face1, Point2d uv1, double radius,
        bool trim, bool extend, double tolerance, out Brep[] outBreps0, out Brep[] outBreps1)
    {
        return ComputeServer.Post<Brep[], Brep[], Brep[]>(ApiAddress(), out outBreps0, out outBreps1, face0, uv0, face1,
            uv1, radius, trim, extend, tolerance);
    }
    ...
    /// <param name="extend">
    ///     If true, then when one input surface is longer than the other, the chamfer surface is extended to
    ///     the input surface edges.
    /// </param>
    /// <param name="tolerance">The tolerance. When in doubt, use the document's model absolute tolerance.</param>
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateChamferSurface(BrepFace face0, Point2d uv0, double radius0, BrepFace face1, Point2d uv1,
        double radius1, bool extend, double tolerance)
    {
    ...
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateChamferSurface(BrepFace face0, Point2d uv0, double radius0, BrepFace face1, Point2d uv1,
        double radius1, bool trim, bool extend, double tolerance, out Brep[] outBreps0, out Brep[] outBreps1)
    {
        return ComputeServer.Post<Brep[], Brep[], Brep[]>(ApiAddress(), out outBreps0, out outBreps1, face0, uv0,
            radius0,
            face1, uv1, radius1, trim, extend, tolerance);
    }
    ...
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateFilletEdges(Brep brep, IEnumerable<int> edgeIndices, IEnumerable<double> startRadii,
        IEnumerable<double> endRadii, BlendType blendType, RailType railType, double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), brep, edgeIndices, startRadii, endRadii, blendType, railType,
            tolerance);
    }
    ...
    /// <param name="tolerance">The tolerance to be used to perform calculations.</param>
    /// <returns>Array of Breps if successful.</returns>
    public static Brep[] CreateFilletEdges(Remote<Brep> brep, IEnumerable<int> edgeIndices,
        IEnumerable<double> startRadii, IEnumerable<double> endRadii, BlendType blendType, RailType railType,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), brep, edgeIndices, startRadii, endRadii, blendType, railType,
            tolerance);
    ...
    ///     is true).
    /// </returns>
    public static Brep[] CreateOffsetBrep(Brep brep, double distance, bool solid, bool extend, double tolerance,
        out Brep[] outBlends, out Brep[] outWalls)
    {
        return ComputeServer.Post<Brep[], Brep[], Brep[]>(ApiAddress(), out outBlends, out outWalls, brep, distance,
            solid, extend, tolerance);
    }
    ...
    ///     is true).
    /// </returns>
    public static Brep[] CreateOffsetBrep(Remote<Brep> brep, double distance, bool solid, bool extend, double tolerance,
        out Brep[] outBlends, out Brep[] outWalls)
    {
        return ComputeServer.Post<Brep[], Brep[], Brep[]>(ApiAddress(), out outBlends, out outWalls, brep, distance,
            solid, extend, tolerance);
    ...
    /// </summary>
    /// <returns>
    ///     true if successful, false if everything is removed or if the result has any Brep edges with more than two Brep
    ///     trims.
    /// </returns>
    ...
    public static Brep CreateFromJoinedEdges(Brep brep0, int edgeIndex0, Brep brep1, int edgeIndex1,
    ...
    /// <param name="joinTolerance">The join tolerance.</param>
    /// <returns>The resulting Brep if successful, null on failure.</returns>
    public static Brep CreateFromJoinedEdges(Remote<Brep> brep0, int edgeIndex0, Remote<Brep> brep1, int edgeIndex1,
        double joinTolerance)
    {
        return ComputeServer.Post<Brep>(ApiAddress(), brep0, edgeIndex0, brep1, edgeIndex1, joinTolerance);
    }
    ...
    /// </example>
    public static Brep[] CreateFromLoft(IEnumerable<Curve> curves, Point3d start, Point3d end, LoftType loftType,
        bool closed)
    {
    ...
    ///     <code source='examples\py\ex_loft.py' lang='py' />
    /// </example>
    public static Brep[] CreateFromLoft(Remote<IEnumerable<Curve>> curves, Point3d start, Point3d end,
    ...
    ///     A number of points to use while rebuilding the curves. 0 leaves turns this parameter
    ///     off.
    /// </param>
    /// <returns>
    ///     Constructs a closed surface, continuing the surface past the last curve around to the
    ///     first curve. Available when you have selected three shape curves.
    /// </returns>
    public static Brep[] CreateFromLoftRebuild(IEnumerable<Curve> curves, Point3d start, Point3d end, LoftType loftType,
        bool closed, int rebuildPointCount)
    {
    ...
    ///     first curve. Available when you have selected three shape curves.
    /// </returns>
    public static Brep[] CreateFromLoftRebuild(Remote<IEnumerable<Curve>> curves, Point3d start, Point3d end,
        LoftType loftType, bool closed, int rebuildPointCount)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curves, start, end, loftType, closed, rebuildPointCount);
    }
    ...
    ///     first curve. Available when you have selected three shape curves.
    /// </returns>
    public static Brep[] CreateFromLoftRefit(IEnumerable<Curve> curves, Point3d start, Point3d end, LoftType loftType,
        bool closed, double refitTolerance)
    {
    ...
    ///     first curve. Available when you have selected three shape curves.
    /// </returns>
    public static Brep[] CreateFromLoftRefit(Remote<IEnumerable<Curve>> curves, Point3d start, Point3d end,
        LoftType loftType, bool closed, double refitTolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), curves, start, end, loftType, closed, refitTolerance);
    }
    ...
    public static Brep[] CreateBooleanIntersection(IEnumerable<Brep> firstSet, IEnumerable<Brep> secondSet,
        double tolerance)
    {
    ...
    /// <returns>An array of Brep results or null on failure.</returns>
    /// <remarks>The solid orientation of the breps make a difference when calling this function</remarks>
    public static Brep[] CreateBooleanIntersection(Remote<IEnumerable<Brep>> firstSet,
        Remote<IEnumerable<Brep>> secondSet, double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), firstSet, secondSet, tolerance);
    }
    ...
    /// <remarks>The solid orientation of the breps make a difference when calling this function</remarks>
    public static Brep[] CreateBooleanIntersection(IEnumerable<Brep> firstSet, IEnumerable<Brep> secondSet,
        double tolerance, bool manifoldOnly)
    {
    ...
    /// <remarks>The solid orientation of the breps make a difference when calling this function</remarks>
    public static Brep[] CreateBooleanIntersection(Remote<IEnumerable<Brep>> firstSet,
        Remote<IEnumerable<Brep>> secondSet, double tolerance, bool manifoldOnly)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), firstSet, secondSet, tolerance, manifoldOnly);
    }
    ...
    /// </example>
    public static Brep[] CreateBooleanDifference(IEnumerable<Brep> firstSet, IEnumerable<Brep> secondSet,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), firstSet, secondSet, tolerance);
    }
    ...
    ///     <code source='examples\py\ex_booleandifference.py' lang='py' />
    /// </example>
    public static Brep[] CreateBooleanDifference(Remote<IEnumerable<Brep>> firstSet,
        Remote<IEnumerable<Brep>> secondSet, double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), firstSet, secondSet, tolerance);
    }
    ...
    /// <param name="tolerance">
    ///     Tolerance to use for splitting operation. When in doubt, use the document's model absolute
    ///     tolerance.
    /// </param>
    ...
    /// </param>
    /// <returns>An array of Brep if successful, an empty array on failure.</returns>
    public static Brep[] CreateBooleanSplit(Remote<IEnumerable<Brep>> firstSet, Remote<IEnumerable<Brep>> secondSet,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), firstSet, secondSet, tolerance);
    }
    ...
    /// <param name="facesToRemove">
    ///     The indices of the Brep faces to remove. These surfaces are removed and the remainder is
    ///     offset inward, using the outer parts of the removed surfaces to join the inner and outer parts.
    /// </param>
    /// <param name="distance">
    ///     The distance, or thickness, for the shell. This is a signed distance value with respect to face
    ///     normals and flipped faces.
    /// </param>
    ...
    /// <param name="tolerance">The offset tolerance. When in doubt, use the document's absolute tolerance.</param>
    /// <returns>An array of Brep results or null on failure.</returns>
    public static Brep[] CreateShell(Remote<Brep> brep, IEnumerable<int> facesToRemove, double distance,
        double tolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), brep, facesToRemove, distance, tolerance);
    }
    ...
    /// </example>
    public static Curve[] CreateContourCurves(Brep brepToContour, Point3d contourStart, Point3d contourEnd,
        double interval)
    {
    ...
    ///     <code source='examples\py\ex_makerhinocontours.py' lang='py' />
    /// </example>
    public static Curve[] CreateContourCurves(Remote<Brep> brepToContour, Point3d contourStart, Point3d contourEnd,
        double interval)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), brepToContour, contourStart, contourEnd, interval);
    }
    ...
    ///     Call compact on the last Join.
    /// </remarks>
    public static bool Join(Remote<Brep> brep, out Brep updatedInstance, Remote<Brep> otherBrep, double tolerance,
        bool compact)
    {
        return ComputeServer.Post<bool, Brep>(ApiAddress(), out updatedInstance, brep, otherBrep, tolerance, compact);
    }
    ...
    {
        return ComputeServer.Post<Brep[], bool>(ApiAddress(), out toleranceWasRaised, brep, cutter,
    ...
    /// <returns>A new array of Breps. This array can be empty.</returns>
    /// <remarks>
    ///     A Curve in cutters is extruded to produce a surface to use as a cutter. The extrusion direction is chosen, as in
    ///     the Rhino Split command,
    ///     based on the properties of the active view. In particular the construction plane Normal and whether the active view
    ///     is a plan view,
    ///     a parallel projection with construction plane normal as the view direction. If planView is false and the curve is
    ///     planar then the curve
    ///     is extruded perpendicular to the curve, otherwise the curve is extruded in the normal direction.
    /// </remarks>
    public static Brep[] Split(this Brep brep, IEnumerable<GeometryBase> cutters, Vector3d normal, bool planView,
    ...
    ///     is extruded perpendicular to the curve, otherwise the curve is extruded in the normal direction.
    /// </remarks>
    public static Brep[] Split(Remote<Brep> brep, Remote<IEnumerable<GeometryBase>> cutters, Vector3d normal,
        bool planView, double intersectionTolerance)
    {
        return ComputeServer.Post<Brep[]>(ApiAddress(), brep, cutters, normal, planView, intersectionTolerance);
    }
    ...
    public static bool JoinEdges(this Brep brep, out Brep updatedInstance, int edgeIndex0, int edgeIndex1,
        double joinTolerance, bool compact)
    {
        return ComputeServer.Post<bool, Brep>(ApiAddress(), out updatedInstance, brep, edgeIndex0, edgeIndex1,
            joinTolerance, compact);
    }
    ...
    /// </param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool JoinEdges(Remote<Brep> brep, out Brep updatedInstance, int edgeIndex0, int edgeIndex1,
        double joinTolerance, bool compact)
    {
        return ComputeServer.Post<bool, Brep>(ApiAddress(), out updatedInstance, brep, edgeIndex0, edgeIndex1,
            joinTolerance, compact);
    ...
    /// <param name="tolerance">
    ///     The desired fitting tolerance to use when bending faces that share edges with both fixed and
    ///     transformed components.
    /// </param>
    /// <param name="timeLimit">
    ///     If the deformation is extreme, it can take a long time to calculate the result.
    ///     If time_limit > 0, then the value specifies the maximum amount of time in seconds you want to spend before giving
    ///     up.
    /// </param>
    /// <param name="useMultipleThreads">True if multiple threads can be used.</param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool TransformComponent(this Brep brep, out Brep updatedInstance,
        IEnumerable<ComponentIndex> components, Transform xform, double tolerance, double timeLimit,
        bool useMultipleThreads)
    {
        return ComputeServer.Post<bool, Brep>(ApiAddress(), out updatedInstance, brep, components, xform, tolerance,
            timeLimit, useMultipleThreads);
    }
    ...
    /// <param name="useMultipleThreads">True if multiple threads can be used.</param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool TransformComponent(Remote<Brep> brep, out Brep updatedInstance,
        IEnumerable<ComponentIndex> components, Transform xform, double tolerance, double timeLimit,
        bool useMultipleThreads)
    {
        return ComputeServer.Post<bool, Brep>(ApiAddress(), out updatedInstance, brep, components, xform, tolerance,
...
public static class BrepFaceCompute
{
    ...
    /// <param name="tolerance">
    ///     Tolerance for pulling operation. Only points that are closer than tolerance will be pulled to
    ///     the face.
    /// </param>
    ...
    /// <returns>True if successful, false otherwise.</returns>
    public static bool DraftAnglePoint(this BrepFace brepface, Point2d testPoint, double testAngle,
        Vector3d pullDirection, bool edge, out Point3d draftPoint, out double draftAngle)
    {
        return ComputeServer.Post<bool, Point3d, double>(ApiAddress(), out draftPoint, out draftAngle, brepface,
            testPoint, testAngle, pullDirection, edge);
    }
    ...
    /// </param>
    /// <returns>true on success.</returns>
    public static bool RebuildEdges(this BrepFace brepface, out BrepFace updatedInstance, double tolerance,
        bool rebuildSharedEdges, bool rebuildVertices)
    {
        return ComputeServer.Post<bool, BrepFace>(ApiAddress(), out updatedInstance, brepface, tolerance,
            rebuildSharedEdges, rebuildVertices);
    }
}
...
public static class CurveCompute
{
    ...
    /// <param name="angleTolerance">
    ///     The angle tolerance in radians. When in doubt, use the document's model space angle
    ///     tolerance.
    /// </param>
    ...
    /// </param>
    /// <returns>The filleted curve if successful. null on failure.</returns>
    public static Curve CreateFilletCornersCurve(Remote<Curve> curve, double radius, double tolerance,
        double angleTolerance)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve, radius, tolerance, angleTolerance);
    }
    ...
    public static Curve CreateArcBlend(Point3d startPt, Vector3d startDir, Point3d endPt, Vector3d endDir,
    ...
    /// <returns>A curve representing the blend between A and B or null on failure.</returns>
    public static Curve CreateBlendCurve(Curve curveA, Curve curveB, BlendContinuity continuity, double bulgeA,
        double bulgeB)
    {
    ...
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve0, t0, reverse0, continuity0, curve1, t1, reverse1,
            continuity1);
    }
    ...
    /// <param name="continuity1">Continuity for the blend at the end</param>
    /// <returns>The blend curve on success. null on failure</returns>
    public static Curve CreateBlendCurve(Remote<Curve> curve0, double t0, bool reverse0, BlendContinuity continuity0,
        Remote<Curve> curve1, double t1, bool reverse1, BlendContinuity continuity1)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve0, t0, reverse0, continuity0, curve1, t1, reverse1,
            continuity1);
    }
    ...
    /// <summary>
    ///     Creates curves between two open or closed input curves. Uses the control points of the curves for finding tween
    ///     curves.
    ...
    ///     This is how the algorithm works: Divides the two curves into an equal number of points, finds the midpoint between
    ///     the
    ///     corresponding points on the curves and interpolates the tween curve through those points. There is no matching of
    ///     curves
    ...
    /// <param name="numSamples">Number of sample points along input curves.</param>
    /// <returns>>An array of joint curves. This array can be empty.</returns>
    public static Curve[] CreateTweenCurvesWithSampling(Remote<Curve> curve0, Remote<Curve> curve1, int numCurves,
        int numSamples)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve0, curve1, numCurves, numSamples);
    }
    ...
    /// <returns>>An array of joint curves. This array can be empty.</returns>
    public static Curve[] CreateTweenCurvesWithSampling(Remote<Curve> curve0, Remote<Curve> curve1, int numCurves,
        int numSamples, double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve0, curve1, numCurves, numSamples, tolerance);
    }
    ...
    /// </param>
    /// <returns>true on success.</returns>
    public static bool MakeEndsMeet(Remote<Curve> curveA, bool adjustStartCurveA, Remote<Curve> curveB,
    ...
    /// <param name="t1Base">Parameter on curve1 where the fillet ought to end (approximately).</param>
    /// <returns>The fillet arc on success, or Arc.Unset on failure.</returns>
    public static Arc CreateFillet(Remote<Curve> curve0, Remote<Curve> curve1, double radius, double t0Base,
        double t1Base)
    {
        return ComputeServer.Post<Arc>(ApiAddress(), curve0, curve1, radius, t0Base, t1Base);
    }
    ...
    /// </example>
    public static Curve[] CreateFilletCurves(Curve curve0, Point3d point0, Curve curve1, Point3d point1, double radius,
        bool join, bool trim, bool arcExtension, double tolerance, double angleTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve0, point0, curve1, point1, radius, join, trim,
            arcExtension,
    ...
    ///     <code source='examples\py\ex_filletcurves.py' lang='py' />
    /// </example>
    public static Curve[] CreateFilletCurves(Remote<Curve> curve0, Point3d point0, Remote<Curve> curve1, Point3d point1,
        double radius, bool join, bool trim, bool arcExtension, double tolerance, double angleTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve0, point0, curve1, point1, radius, join, trim,
            arcExtension,
            tolerance, angleTolerance);
    ...
    /// <summary>
    ///     Creates outline curves created from a text string. The functionality is similar to what you find in Rhino's
    ///     TextObject command or TextEntity.Explode() in RhinoCommon.
    /// </summary>
    ...
    ///     The font style. The font style can be any number of the following: 0 - Normal, 1 - Bold, 2 -
    ///     Italic
    /// </param>
    /// <param name="closeLoops">
    ///     Set this value to True when dealing with normal fonts and when you expect closed loops. You
    ///     may want to set this to False when specifying a single-stroke font where you don't want closed loops.
    /// </param>
    /// <param name="plane">The plane on which the outline curves will lie.</param>
    /// <param name="smallCapsScale">
    ///     Displays lower-case letters as small caps. Set the relative text size to a percentage of
    ///     the normal text.
    /// </param>
    ...
    public static Curve[] CreateTextOutlines(string text, string font, double textHeight, int textStyle,
        bool closeLoops, Plane plane, double smallCapsScale, double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), text, font, textHeight, textStyle, closeLoops, plane,
            smallCapsScale, tolerance);
    }
    ...
    /// <returns>An array containing one or more curves if successful.</returns>
    public static Curve[] CreateCurve2View(Curve curveA, Curve curveB, Vector3d vectorA, Vector3d vectorB,
    ...
    /// <param name="angleTolerance">The angle tolerance for the operation.</param>
    /// <returns>An array containing one or more curves if successful.</returns>
    public static Curve[] CreateCurve2View(Remote<Curve> curveA, Remote<Curve> curveB, Vector3d vectorA,
        Vector3d vectorB, double tolerance, double angleTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curveA, curveB, vectorA, vectorB, tolerance, angleTolerance);
    }
    ...
    /// <param name="tolerance">A tolerance value.</param>
    /// <returns>A curve array.</returns>
    public static Curve[] ProjectToMesh(Remote<Curve> curve, Remote<IEnumerable<Mesh>> meshes, Vector3d direction,
        double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, meshes, direction, tolerance);
    }
    ...
    /// <param name="tolerance">A tolerance value.</param>
    /// <returns>A curve array.</returns>
    public static Curve[] ProjectToMesh(IEnumerable<Curve> curves, IEnumerable<Mesh> meshes, Vector3d direction,
        double tolerance)
    {
    ...
    /// <param name="tolerance">A tolerance value.</param>
    /// <returns>A curve array.</returns>
    public static Curve[] ProjectToMesh(Remote<IEnumerable<Curve>> curves, Remote<IEnumerable<Mesh>> meshes,
        Vector3d direction, double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curves, meshes, direction, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance to use for projection.</param>
    /// <returns>An array of projected curves or empty array if the projection set is empty.</returns>
    public static Curve[] ProjectToBrep(Remote<Curve> curve, Remote<IEnumerable<Brep>> breps, Vector3d direction,
        double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, breps, direction, tolerance);
    }
    ...
    /// <returns>An array of projected curves or null if the projection set is empty.</returns>
    public static Curve[] ProjectToBrep(Remote<Curve> curve, Remote<IEnumerable<Brep>> breps, Vector3d direction,
        double tolerance, out int[] brepIndices)
    {
        return ComputeServer.Post<Curve[], int[]>(ApiAddress(), out brepIndices, curve, breps, direction, tolerance);
    }
    ...
    /// <param name="tolerance">Tolerance to use for projection.</param>
    /// <returns>An array of projected curves or empty array if the projection set is empty.</returns>
    public static Curve[] ProjectToBrep(IEnumerable<Curve> curves, IEnumerable<Brep> breps, Vector3d direction,
        double tolerance)
    {
    ...
    /// <param name="tolerance">Tolerance to use for projection.</param>
    /// <returns>An array of projected curves or empty array if the projection set is empty.</returns>
    public static Curve[] ProjectToBrep(Remote<IEnumerable<Curve>> curves, Remote<IEnumerable<Brep>> breps,
        Vector3d direction, double tolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curves, breps, direction, tolerance);
    }
    ...
    public static Curve[] ProjectToBrep(IEnumerable<Curve> curves, IEnumerable<Brep> breps, Vector3d direction,
        double tolerance, out int[] curveIndices, out int[] brepIndices)
    {
        return ComputeServer.Post<Curve[], int[], int[]>(ApiAddress(), out curveIndices, out brepIndices, curves, breps,
            direction, tolerance);
    }
    ...
    /// <returns>An array of projected curves. Array is empty if the projection set is empty.</returns>
    public static Curve[] ProjectToBrep(Remote<IEnumerable<Curve>> curves, Remote<IEnumerable<Brep>> breps,
        Vector3d direction, double tolerance, out int[] curveIndices, out int[] brepIndices)
    {
        return ComputeServer.Post<Curve[], int[], int[]>(ApiAddress(), out curveIndices, out brepIndices, curves, breps,
            direction, tolerance);
    }
    ...
    /// </returns>
    public static RegionContainment PlanarClosedCurveRelationship(Curve curveA, Curve curveB, Plane testPlane,
        double tolerance)
    {
    ...
    ///     A value indicating the relationship between the first and the second curve.
    /// </returns>
    public static RegionContainment PlanarClosedCurveRelationship(Remote<Curve> curveA, Remote<Curve> curveB,
        Plane testPlane, double tolerance)
    {
        return ComputeServer.Post<RegionContainment>(ApiAddress(), curveA, curveB, testPlane, tolerance);
    }
    ...
    /// <param name="tolerance">A tolerance value for intersection.</param>
    /// <returns>true if the curves intersect, otherwise false</returns>
    public static bool PlanarCurveCollision(Remote<Curve> curveA, Remote<Curve> curveB, Plane testPlane,
        double tolerance)
    {
        return ComputeServer.Post<bool>(ApiAddress(), curveA, curveB, testPlane, tolerance);
    }
    ...
    ///     <para>Use 0 to turn off this parameter.</para>
    /// </param>
    /// <returns>true on success, false on failure.</returns>
    ...
    public static bool ClosestPoints(this Curve curve, Curve otherCurve, out Point3d pointOnThisCurve,
        out Point3d pointOnOtherCurve)
    {
        return ComputeServer.Post<bool, Point3d, Point3d>(ApiAddress(), out pointOnThisCurve, out pointOnOtherCurve,
            curve, otherCurve);
    }
    ...
    /// <param name="pointOnOtherCurve">The point on other curve. This out parameter is assigned during this call.</param>
    /// <returns>true on success; false on error.</returns>
    public static bool ClosestPoints(Remote<Curve> curve, Remote<Curve> otherCurve, out Point3d pointOnThisCurve,
        out Point3d pointOnOtherCurve)
    {
        return ComputeServer.Post<bool, Point3d, Point3d>(ApiAddress(), out pointOnThisCurve, out pointOnOtherCurve,
            curve, otherCurve);
    ...
    ///     If false, control point locations are not changed or changed minimally (only one point may move) and only the knot
    ///     vector is altered.
    /// </param>
    /// <returns>The resulting curve if successful, null otherwise.</returns>
    ...
    /// <returns>true on success, false on failure.</returns>
    public static bool LengthParameter(Remote<Curve> curve, double segmentLength, out double t,
        double fractionalTolerance, Interval subdomain)
    {
        return ComputeServer.Post<bool, double>(ApiAddress(), out t, curve, segmentLength, fractionalTolerance,
            subdomain);
    }
    ...
    /// </param>
    /// <returns>
    ///     If successful, array of curve parameters such that the length of the curve from its start to t[i] is
    ///     s[i]*curve_length.
    ///     Null on failure.
    /// </returns>
    ...
    /// <param name="includeEnds">If true, then the point at the start of the first division segment is returned.</param>
    /// <param name="points">
    ///     A list of division points. If the function returns successfully, this point-array will be filled
    ///     in.
    /// </param>
    ...
    /// <param name="interval">A distance to measure on the contouring axis.</param>
    /// <returns>An array of points; or null on error.</returns>
    public static Point3d[] DivideAsContour(Remote<Curve> curve, Point3d contourStart, Point3d contourEnd,
        double interval)
    {
        return ComputeServer.Post<Point3d[]>(ApiAddress(), curve, contourStart, contourEnd, interval);
    }
    ...
    /// </summary>
    /// <param name="t0">
    ///     Start of extension domain, if the start is not inside the
    ///     Domain of this curve, an attempt will be made to extend the curve.
    /// </param>
    /// <param name="t1">
    ///     End of extension domain, if the end is not inside the
    ///     Domain of this curve, an attempt will be made to extend the curve.
    /// </param>
    ...
    public static Curve Simplify(this Curve curve, CurveSimplifyOptions options, double distanceTolerance,
        double angleToleranceRadians)
    {
    ...
    /// <param name="angleToleranceRadians">An angle tolerance for the simplification.</param>
    /// <returns>New simplified curve on success, null on failure.</returns>
    public static Curve Simplify(Remote<Curve> curve, CurveSimplifyOptions options, double distanceTolerance,
        double angleToleranceRadians)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve, options, distanceTolerance, angleToleranceRadians);
    }
    ...
    /// </summary>
    /// <param name="end">
    ///     If CurveEnd.Start the function simplifies the last two start
    ...
    /// <param name="angleToleranceRadians">An angle tolerance for the simplification.</param>
    /// <returns>New simplified curve on success, null on failure.</returns>
    public static Curve SimplifyEnd(this Curve curve, CurveEnd end, CurveSimplifyOptions options,
        double distanceTolerance, double angleToleranceRadians)
    {
    ...
    /// <param name="angleToleranceRadians">An angle tolerance for the simplification.</param>
    /// <returns>New simplified curve on success, null on failure.</returns>
    public static Curve SimplifyEnd(Remote<Curve> curve, CurveEnd end, CurveSimplifyOptions options,
        double distanceTolerance, double angleToleranceRadians)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve, end, options, distanceTolerance, angleToleranceRadians);
    }
    ...
    /// <param name="angleTolerance">
    ///     (in radians) kinks with angles &lt;= angleTolerance are smoothed out 0.05 is a good
    ///     default.
    /// </param>
    /// <param name="clampStart">
    ...
    public static Curve Fair(this Curve curve, double distanceTolerance, double angleTolerance, int clampStart,
        int clampEnd, int iterations)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve, distanceTolerance, angleTolerance, clampStart, clampEnd,
            iterations);
    }
    ...
    /// <param name="iterations">The number of iterations to use in adjusting the curve.</param>
    /// <returns>Returns new faired Curve on success, null on failure.</returns>
    public static Curve Fair(Remote<Curve> curve, double distanceTolerance, double angleTolerance, int clampStart,
        int clampEnd, int iterations)
    {
        return ComputeServer.Post<Curve>(ApiAddress(), curve, distanceTolerance, angleTolerance, clampStart, clampEnd,
            iterations);
    ...
    /// <param name="fitTolerance">
    ///     The fitting tolerance. If fitTolerance is RhinoMath.UnsetValue or &lt;=0.0,
    ...
    ///     <para>
    ///         If angleTolerance is RhinoMath.UnsetValue or &lt;0.0, the document angle tolerance is used for the kink
    ///         smoothing
    ///     </para>
    /// </param>
    ...
    ///     If maxEdgeLength = 0, the parameter
    ///     is ignored. This parameter controls the maximum permitted edge length.
    /// </param>
    /// <param name="keepStartPoint">
    ///     If true the starting point of the curve
    ...
    public static PolylineCurve ToPolyline(this Curve curve, int mainSegmentCount, int subSegmentCount,
        double maxAngleRadians, double maxChordLengthRatio, double maxAspectRatio, double tolerance,
        double minEdgeLength,
        double maxEdgeLength, bool keepStartPoint)
    {
        return ComputeServer.Post<PolylineCurve>(ApiAddress(), curve, mainSegmentCount, subSegmentCount,
            maxAngleRadians,
            maxChordLengthRatio, maxAspectRatio, tolerance, minEdgeLength, maxEdgeLength, keepStartPoint);
    }
    ...
    /// </param>
    /// <returns>PolylineCurve on success, null on error.</returns>
    public static PolylineCurve ToPolyline(Remote<Curve> curve, int mainSegmentCount, int subSegmentCount,
        double maxAngleRadians, double maxChordLengthRatio, double maxAspectRatio, double tolerance,
        double minEdgeLength,
        double maxEdgeLength, bool keepStartPoint)
    {
    ...
    public static PolylineCurve ToPolyline(this Curve curve, int mainSegmentCount, int subSegmentCount,
        double maxAngleRadians, double maxChordLengthRatio, double maxAspectRatio, double tolerance,
        double minEdgeLength,
        double maxEdgeLength, bool keepStartPoint, Interval curveDomain)
    {
        return ComputeServer.Post<PolylineCurve>(ApiAddress(), curve, mainSegmentCount, subSegmentCount,
            maxAngleRadians,
            maxChordLengthRatio, maxAspectRatio, tolerance, minEdgeLength, maxEdgeLength, keepStartPoint, curveDomain);
    }
    ...
    /// </summary>
    /// <param name="tolerance">
    ///     The tolerance. This is the maximum deviation from line midpoints to the curve. When in doubt,
    ///     use the document's model space absolute tolerance.
    /// </param>
    /// <param name="angleTolerance">
    ///     The angle tolerance in radians. This is the maximum deviation of the line directions. When
    ///     in doubt, use the document's model space angle tolerance.
    /// </param>
    ...
    public static PolylineCurve ToPolyline(this Curve curve, double tolerance, double angleTolerance,
        double minimumLength, double maximumLength)
    {
        return ComputeServer.Post<PolylineCurve>(ApiAddress(), curve, tolerance, angleTolerance, minimumLength,
            maximumLength);
    }
    ...
    /// <param name="maximumLength">The maximum segment length.</param>
    /// <returns>PolyCurve on success, null on error.</returns>
    public static PolylineCurve ToPolyline(Remote<Curve> curve, double tolerance, double angleTolerance,
        double minimumLength, double maximumLength)
    {
        return ComputeServer.Post<PolylineCurve>(ApiAddress(), curve, tolerance, angleTolerance, minimumLength,
            maximumLength);
    ...
    /// <summary>
    ///     Converts a curve into polycurve consisting of arc segments. Sections of the input curves that are nearly straight
    ///     are converted to straight-line segments.
    /// </summary>
    /// <param name="tolerance">
    ///     The tolerance. This is the maximum deviation from arc midpoints to the curve. When in doubt,
    ///     use the document's model space absolute tolerance.
    /// </param>
    /// <param name="angleTolerance">
    ///     The angle tolerance in radians. This is the maximum deviation of the arc end directions
    ///     from the curve direction. When in doubt, use the document's model space angle tolerance.
    /// </param>
    /// <param name="minimumLength">The minimum segment length.</param>
    /// <param name="maximumLength">The maximum segment length.</param>
    /// <returns>PolyCurve on success, null on error.</returns>
    public static PolyCurve ToArcsAndLines(this Curve curve, double tolerance, double angleTolerance,
        double minimumLength, double maximumLength)
    {
        return ComputeServer.Post<PolyCurve>(ApiAddress(), curve, tolerance, angleTolerance, minimumLength,
            maximumLength);
    }
    ...
    /// <param name="maximumLength">The maximum segment length.</param>
    /// <returns>PolyCurve on success, null on error.</returns>
    public static PolyCurve ToArcsAndLines(Remote<Curve> curve, double tolerance, double angleTolerance,
        double minimumLength, double maximumLength)
    {
        return ComputeServer.Post<PolyCurve>(ApiAddress(), curve, tolerance, angleTolerance, minimumLength,
            maximumLength);
    ...
    public static Curve[] Offset(this Curve curve, Plane plane, double distance, double tolerance,
    ...
    /// <param name="cornerStyle">Corner style for offset kinks.</param>
    /// <returns>Offset curves on success, null on failure.</returns>
    public static Curve[] Offset(Remote<Curve> curve, Plane plane, double distance, double tolerance,
        CurveOffsetCornerStyle cornerStyle)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, plane, distance, tolerance, cornerStyle);
    }
    ...
    /// <param name="cornerStyle">Corner style for offset kinks.</param>
    /// <returns>Offset curves on success, null on failure.</returns>
    public static Curve[] Offset(this Curve curve, Point3d directionPoint, Vector3d normal, double distance,
        double tolerance, CurveOffsetCornerStyle cornerStyle)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, directionPoint, normal, distance, tolerance,
            cornerStyle);
    }
    ...
    /// <param name="cornerStyle">Corner style for offset kinks.</param>
    /// <returns>Offset curves on success, null on failure.</returns>
    public static Curve[] Offset(Remote<Curve> curve, Point3d directionPoint, Vector3d normal, double distance,
        double tolerance, CurveOffsetCornerStyle cornerStyle)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, directionPoint, normal, distance, tolerance,
            cornerStyle);
    ...
    public static Curve RibbonOffset(this Curve curve, out Curve updatedInstance, double distance, double blendRadius,
        Point3d directionPoint, Vector3d normal, double tolerance)
    {
        return ComputeServer.Post<Curve, Curve>(ApiAddress(), out updatedInstance, curve, distance, blendRadius,
            directionPoint, normal, tolerance);
    }
    ...
    /// <param name="tolerance">Used to determine self-intersections, not offset error.</param>
    /// <returns>The offset curve if successful.</returns>
    public static Curve RibbonOffset(Remote<Curve> curve, out Curve updatedInstance, double distance,
        double blendRadius, Point3d directionPoint, Vector3d normal, double tolerance)
    {
        return ComputeServer.Post<Curve, Curve>(ApiAddress(), out updatedInstance, curve, distance, blendRadius,
            directionPoint, normal, tolerance);
    }
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If face is null.</exception>
    public static Curve[] OffsetOnSurface(this Curve curve, BrepFace face, Point2d throughPoint,
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If face is null.</exception>
    public static Curve[] OffsetOnSurface(Remote<Curve> curve, BrepFace face, Point2d throughPoint,
        double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, face, throughPoint, fittingTolerance);
    }
    ...
    public static Curve[] OffsetOnSurface(this Curve curve, BrepFace face, double[] curveParameters,
        double[] offsetDistances, double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, face, curveParameters, offsetDistances,
            fittingTolerance);
    }
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If face, curveParameters or offsetDistances are null.</exception>
    public static Curve[] OffsetOnSurface(Remote<Curve> curve, BrepFace face, double[] curveParameters,
        double[] offsetDistances, double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, face, curveParameters, offsetDistances,
            fittingTolerance);
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If surface is null.</exception>
    public static Curve[] OffsetOnSurface(Remote<Curve> curve, Remote<Surface> surface, double distance,
        double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, surface, distance, fittingTolerance);
    }
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If surface is null.</exception>
    public static Curve[] OffsetOnSurface(this Curve curve, Surface surface, Point2d throughPoint,
        double fittingTolerance)
    {
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If surface is null.</exception>
    public static Curve[] OffsetOnSurface(Remote<Curve> curve, Remote<Surface> surface, Point2d throughPoint,
        double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, surface, throughPoint, fittingTolerance);
    }
    ...
    public static Curve[] OffsetOnSurface(this Curve curve, Surface surface, double[] curveParameters,
        double[] offsetDistances, double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, surface, curveParameters, offsetDistances,
            fittingTolerance);
    }
    ...
    /// <returns>Offset curves on success, or null on failure.</returns>
    /// <exception cref="ArgumentNullException">If surface, curveParameters or offsetDistances are null.</exception>
    public static Curve[] OffsetOnSurface(Remote<Curve> curve, Remote<Surface> surface, double[] curveParameters,
        double[] offsetDistances, double fittingTolerance)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), curve, surface, curveParameters, offsetDistances,
            fittingTolerance);
...
public static class GeometryBaseCompute
{
    ...
    /// </param>
    /// <returns>
    ///     The accurate bounding box of the transformed geometry in world coordinates
...
public static class AreaMassPropertiesCompute
{
    ...
    {
        return ComputeServer.Post<AreaMassProperties>(ApiAddress("compute-curve_double"), closedPlanarCurve,
            planarTolerance);
    }
    ...
    /// <returns>The AreaMassProperties for the given Mesh or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When mesh is null.</exception>
    public static AreaMassProperties Compute(Mesh mesh, bool area, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
    ...
    /// <returns>The AreaMassProperties for the given Brep or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When brep is null.</exception>
    public static AreaMassProperties Compute(Brep brep, bool area, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
        return ComputeServer.Post<AreaMassProperties>(ApiAddress(
    ...
    /// <returns>The AreaMassProperties for the given Surface or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When surface is null.</exception>
    public static AreaMassProperties Compute(Surface surface, bool area, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
        return ComputeServer.Post<AreaMassProperties>(ApiAddress(
    ...
    public static AreaMassProperties Compute(IEnumerable<GeometryBase> geometry, bool area, bool firstMoments,
        bool secondMoments, bool productMoments)
    {
        return ComputeServer.Post<AreaMassProperties>(ApiAddress(
...
public static class VolumeMassPropertiesCompute
{
    ...
    /// <returns>The VolumeMassProperties for the given Mesh or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When Mesh is null.</exception>
    public static VolumeMassProperties Compute(Mesh mesh, bool volume, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
    ...
    /// <returns>The VolumeMassProperties for the given Brep or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When brep is null.</exception>
    public static VolumeMassProperties Compute(Brep brep, bool volume, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
        return ComputeServer.Post<VolumeMassProperties>(ApiAddress(
            "compute-brep_bool_bool_bool_bool"
    ...
    /// <returns>The VolumeMassProperties for the given Surface or null on failure.</returns>
    /// <exception cref="System.ArgumentNullException">When surface is null.</exception>
    public static VolumeMassProperties Compute(Surface surface, bool volume, bool firstMoments, bool secondMoments,
        bool productMoments)
    {
        return ComputeServer.Post<VolumeMassProperties>(ApiAddress(
            "compute-surface_bool_bool_bool_bool"
    ...
    /// <param name="productMoments">true to calculate volume product moments.</param>
    /// <returns>The VolumeMassProperties for the entire collection or null on failure.</returns>
    public static VolumeMassProperties Compute(IEnumerable<GeometryBase> geometry, bool volume, bool firstMoments,
        bool secondMoments, bool productMoments)
    {
        return ComputeServer.Post<VolumeMassProperties>(ApiAddress(
            "compute-geometrybasearray_bool_bool_bool_bool"
...
public static class MeshCompute
{
    ...
    /// <param name="capTop">If true end at Cylinder.Height2 should be capped.</param>
    /// <param name="quadCaps">
    ///     If true and it's possible to make quad caps, i.e.. around is even, then caps will have quad
    ///     faces.
    /// </param>
    /// <exception cref="ArgumentException">Thrown when cylinder is invalid.</exception>
    /// <returns>Returns a mesh cylinder if successful, null otherwise.</returns>
    ...
    ///     If true, the mesh might have more vertices than the list of input points,
    ///     if doing so will improve long thin triangles.
    /// </param>
    /// <param name="edges">
    ///     A list of polylines, or other lists of points representing edges.
    ...
    public static Mesh CreateFromTessellation(IEnumerable<Point3d> points, IEnumerable<IEnumerable<Point3d>> edges,
        Plane plane, bool allowNewVertices)
    {
    ...
    /// </summary>
    /// <param name="outerBoundary">
    ///     (optional: can be null) Outer boundary
    ...
    public static Mesh CreatePatch(Polyline outerBoundary, double angleToleranceRadians, Surface pullbackSurface,
        IEnumerable<Curve> innerBoundaryCurves, IEnumerable<Curve> innerBothSideCurves,
        IEnumerable<Point3d> innerPoints,
        bool trimback, int divisions)
    {
        return ComputeServer.Post<Mesh>(ApiAddress(), outerBoundary, angleToleranceRadians, pullbackSurface,
            innerBoundaryCurves, innerBothSideCurves, innerPoints, trimback, divisions);
    }
    ...
    /// </param>
    /// <returns>mesh on success; null on failure</returns>
    public static Mesh CreatePatch(Polyline outerBoundary, double angleToleranceRadians,
        Remote<Surface> pullbackSurface, Remote<IEnumerable<Curve>> innerBoundaryCurves,
        Remote<IEnumerable<Curve>> innerBothSideCurves, IEnumerable<Point3d> innerPoints, bool trimback, int divisions)
    {
        return ComputeServer.Post<Mesh>(ApiAddress(), outerBoundary, angleToleranceRadians, pullbackSurface,
            innerBoundaryCurves, innerBothSideCurves, innerPoints, trimback, divisions);
    }
    ...
    /// <param name="secondSet">Second set of Meshes (the set to subtract).</param>
    /// <returns>An array of Mesh results or null on failure.</returns>
    public static Mesh[] CreateBooleanDifference(Remote<IEnumerable<Mesh>> firstSet,
        Remote<IEnumerable<Mesh>> secondSet)
    {
        return ComputeServer.Post<Mesh[]>(ApiAddress(), firstSet, secondSet);
    }
    ...
    /// <param name="secondSet">Second set of Meshes.</param>
    /// <returns>An array of Mesh results or null on failure.</returns>
    public static Mesh[] CreateBooleanIntersection(Remote<IEnumerable<Mesh>> firstSet,
        Remote<IEnumerable<Mesh>> secondSet)
    {
        return ComputeServer.Post<Mesh[]>(ApiAddress(), firstSet, secondSet);
    }
    ...
    /// </summary>
    /// <param name="meshesToSplit">
    ///     A list, an array, or any enumerable set of meshes to be split. If this is null, null will
    ///     be returned.
    /// </param>
    /// <param name="meshSplitters">
    ///     A list, an array, or any enumerable set of meshes that cut. If this is null, null will be
    ///     returned.
    /// </param>
    ...
    /// </param>
    /// <returns>A new mesh array, or null on error.</returns>
    public static Mesh[] CreateBooleanSplit(Remote<IEnumerable<Mesh>> meshesToSplit,
        Remote<IEnumerable<Mesh>> meshSplitters)
    {
        return ComputeServer.Post<Mesh[]>(ApiAddress(), meshesToSplit, meshSplitters);
    }
    ...
    /// <param name="boundingBox">The bounding box controls the length of the extrusion.</param>
    /// <returns>A new mesh, or null on failure.</returns>
    public static Mesh CreateFromCurveExtrusion(Remote<Curve> curve, Vector3d direction, MeshingParameters parameters,
        BoundingBox boundingBox)
    {
        return ComputeServer.Post<Mesh>(ApiAddress(), curve, direction, parameters, boundingBox);
    }
    ...
    /// <returns>
    ///     A valid meshes array if successful. If no change was required, some meshes can be null. Otherwise, null, when
    ///     no changes were done.
    /// </returns>
    /// <remarks>
    ///     <seealso cref="RequireIterativeCleanup" />
    /// </remarks>
    ...
    /// <remarks>
    ///     <seealso cref="Rhino.Geometry.Mesh.CreateFromIterativeCleanup" />
    /// </remarks>
    ...
    /// <param name="modifyNormals">
    ///     If true, the vertex normals on each side of the edge take the same value as the face to which they belong, giving
    ///     the mesh a hard edge look.
    ///     If false, each of the vertex normals on either side of the edge is assigned the same value as the original normal
    ///     that the pair is replacing, keeping a smooth look.
    /// </param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool UnweldEdge(this Mesh mesh, out Mesh updatedInstance, IEnumerable<int> edgeIndices,
        bool modifyNormals)
    {
    ...
    /// </param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool UnweldEdge(Remote<Mesh> mesh, out Mesh updatedInstance, IEnumerable<int> edgeIndices,
        bool modifyNormals)
    {
        return ComputeServer.Post<bool, Mesh>(ApiAddress(), out updatedInstance, mesh, edgeIndices, modifyNormals);
    }
    ...
    /// <returns>true if successful, false otherwise.</returns>
    /// <remarks>
    ///     This function does not differentiate between inner and outer naked edges.
    ...
    /// <returns>
    ///     If countOnly=false, the number of faces that were modified. If countOnly=true, the number of faces that would
    ///     be modified.
    /// </returns>
    ...
    ///     -1 if not successful; the value of pointOnMesh is undefined.
    /// </returns>
    public static int ClosestPoint(Remote<Mesh> mesh, Point3d testPoint, out Point3d pointOnMesh,
    ...
    ///     -1 if not successful; the value of pointOnMesh is undefined.
    /// </returns>
    public static int ClosestPoint(this Mesh mesh, Point3d testPoint, out Point3d pointOnMesh,
        out Vector3d normalAtPoint, double maximumDistance)
    {
        return ComputeServer.Post<int, Point3d, Vector3d>(ApiAddress(), out pointOnMesh, out normalAtPoint, mesh,
            testPoint, maximumDistance);
    }
    ...
    /// <returns>
    ///     A Point on the mesh or Point3d.Unset if the faceIndex is not valid or if the barycentric coordinates could not
    ///     be evaluated.
    /// </returns>
    ...
    /// <param name="meshPoint">MeshPoint instance containing a valid Face Index and Barycentric coordinates.</param>
    /// <returns>
    ///     A Normal vector to the mesh or Vector3d.Unset if the faceIndex is not valid or if the barycentric coordinates
    ...
    /// <summary>
    ///     Gets a polyline approximation of the input curve and then moves its control points to the closest point on the
    ///     mesh.
    ...
    public static Mesh[] SplitWithProjectedPolylines(this Mesh mesh, IEnumerable<PolylineCurve> curves,
        double tolerance)
    {
    ...
    /// <param name="tolerance">A tolerance value.</param>
    /// <returns>An array of meshes, or null if no change would happen.</returns>
    public static Mesh[] SplitWithProjectedPolylines(Remote<Mesh> mesh, IEnumerable<PolylineCurve> curves,
        double tolerance)
    {
        return ComputeServer.Post<Mesh[]>(ApiAddress(), mesh, curves, tolerance);
    }
    ...
    /// </remarks>
    public static int CollapseFacesByEdgeLength(this Mesh mesh, out Mesh updatedInstance, bool bGreaterThan,
        double edgeLength)
    {
    ...
    ///     the input criteria because the lengths of some initial edges may be altered as other edges are collapsed.
    /// </remarks>
    public static int CollapseFacesByEdgeLength(Remote<Mesh> mesh, out Mesh updatedInstance, bool bGreaterThan,
        double edgeLength)
    {
        return ComputeServer.Post<int, Mesh>(ApiAddress(), out updatedInstance, mesh, bGreaterThan, edgeLength);
    }
    ...
    /// </remarks>
    public static int CollapseFacesByArea(this Mesh mesh, out Mesh updatedInstance, double lessThanArea,
    ...
    ///     Use large numbers for lessThanArea or zero for greaterThanArea to simulate an OR.
    /// </remarks>
    public static int CollapseFacesByArea(Remote<Mesh> mesh, out Mesh updatedInstance, double lessThanArea,
        double greaterThanArea)
    {
        return ComputeServer.Post<int, Mesh>(ApiAddress(), out updatedInstance, mesh, lessThanArea, greaterThanArea);
    }
    ...
    /// <summary>
    ///     Collapses a multiple mesh faces, determined by face aspect ratio, based on criteria found in Stan Melax's polygon
    ///     reduction,
    ///     see http://pomax.nihongoresources.com/downloads/PolygonReduction.pdf
    /// </summary>
    ...
    ///     because of an error. The exception message specifies the error.
    /// </exception>
    public static Mesh WithEdgeSoftening(this Mesh mesh, double softeningRadius, bool chamfer, bool faceted, bool force,
    ...
    ///     because of an error. The exception message specifies the error.
    /// </exception>
    public static Mesh WithEdgeSoftening(Remote<Mesh> mesh, double softeningRadius, bool chamfer, bool faceted,
        bool force, double angleThreshold)
    {
        return ComputeServer.Post<Mesh>(ApiAddress(), mesh, softeningRadius, chamfer, faceted, force, angleThreshold);
    }
    ...
    /// <returns></returns>
    public static Mesh QuadRemeshBrep(Remote<Brep> brep, QuadRemeshParameters parameters,
        Remote<IEnumerable<Curve>> guideCurves)
    {
        return ComputeServer.Post<Mesh>(ApiAddress(), brep, parameters, guideCurves);
    }
    ...
    public static bool Reduce(this Mesh mesh, out Mesh updatedInstance, int desiredPolygonCount, bool allowDistortion,
        int accuracy, bool normalizeSize)
    {
        return ComputeServer.Post<bool, Mesh>(ApiAddress(), out updatedInstance, mesh, desiredPolygonCount,
            allowDistortion, accuracy, normalizeSize);
    }
    ...
    /// <param name="normalizeSize">If true mesh is fitted to an axis aligned unit cube until reduction is complete</param>
    /// <returns>True if mesh is successfully reduced and false if mesh could not be reduced for some reason.</returns>
    public static bool Reduce(Remote<Mesh> mesh, out Mesh updatedInstance, int desiredPolygonCount,
        bool allowDistortion, int accuracy, bool normalizeSize)
    {
        return ComputeServer.Post<bool, Mesh>(ApiAddress(), out updatedInstance, mesh, desiredPolygonCount,
            allowDistortion, accuracy, normalizeSize);
    }
    ...
    /// <param name="normalizeSize">If true mesh is fitted to an axis aligned unit cube until reduction is complete</param>
    /// <param name="threaded">
    ///     If True then will run computation inside a worker thread and ignore any provided CancellationTokens and
    ///     ProgressReporters.
    ///     If False then will run on main thread.
    /// </param>
    /// <returns>True if mesh is successfully reduced and false if mesh could not be reduced for some reason.</returns>
    public static bool Reduce(this Mesh mesh, out Mesh updatedInstance, int desiredPolygonCount, bool allowDistortion,
        int accuracy, bool normalizeSize, bool threaded)
    {
        return ComputeServer.Post<bool, Mesh>(ApiAddress(), out updatedInstance, mesh, desiredPolygonCount,
            allowDistortion, accuracy, normalizeSize, threaded);
    }
    ...
    /// <returns>True if mesh is successfully reduced and false if mesh could not be reduced for some reason.</returns>
    public static bool Reduce(Remote<Mesh> mesh, out Mesh updatedInstance, int desiredPolygonCount,
        bool allowDistortion, int accuracy, bool normalizeSize, bool threaded)
    {
        return ComputeServer.Post<bool, Mesh>(ApiAddress(), out updatedInstance, mesh, desiredPolygonCount,
            allowDistortion, accuracy, normalizeSize, threaded);
    }
    ...
    /// <param name="maximumThickness">
    ///     Maximum thickness to consider. Use as small a thickness as possible to speed up the
    ///     solver.
    /// </param>
    ...
    ///     <code source='examples\py\ex_makerhinocontours.py' lang='py' />
    /// </example>
    public static Curve[] CreateContourCurves(Mesh meshToContour, Point3d contourStart, Point3d contourEnd,
        double interval)
    {
    ...
    ///     <code source='examples\py\ex_makerhinocontours.py' lang='py' />
    /// </example>
    public static Curve[] CreateContourCurves(Remote<Mesh> meshToContour, Point3d contourStart, Point3d contourEnd,
        double interval)
    {
        return ComputeServer.Post<Curve[]>(ApiAddress(), meshToContour, contourStart, contourEnd, interval);
    }
...
public static class NurbsCurveCompute
{
    ...
    {
        return ComputeServer.Post<NurbsCurve[]>(ApiAddress(), curves, startPt, endPt, simplifyMethod, numPoints,
            refitTolerance, angleTolerance);
    }
    ...
    /// <param name="angleTolerance">The angle tolerance in radians.</param>
    /// <returns>The output NURBS surfaces if successful.</returns>
    public static NurbsCurve[] MakeCompatible(Remote<IEnumerable<Curve>> curves, Point3d startPt, Point3d endPt,
        int simplifyMethod, int numPoints, double refitTolerance, double angleTolerance)
    {
        return ComputeServer.Post<NurbsCurve[]>(ApiAddress(), curves, startPt, endPt, simplifyMethod, numPoints,
            refitTolerance, angleTolerance);
    ...
    /// </param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool SetGrevillePoints(this NurbsCurve nurbscurve, out NurbsCurve updatedInstance,
    ...
    /// </param>
    /// <returns>true if successful, false otherwise.</returns>
    public static bool SetGrevillePoints(Remote<NurbsCurve> nurbscurve, out NurbsCurve updatedInstance,
        IEnumerable<Point3d> points)
    {
        return ComputeServer.Post<bool, NurbsCurve>(ApiAddress(), out updatedInstance, nurbscurve, points);
    }
    ...
    ///     The number of turns in spiral or helix. Positive
    ...
    public static NurbsCurve CreateSpiral(Point3d axisStart, Vector3d axisDir, Point3d radiusPoint, double pitch,
        double turnCount, double radius0, double radius1)
    {
        return ComputeServer.Post<NurbsCurve>(ApiAddress(), axisStart, axisDir, radiusPoint, pitch, turnCount, radius0,
            radius1);
    }
    ...
    {
        return ComputeServer.Post<NurbsCurve>(ApiAddress(), railCurve, t0, t1, radiusPoint, pitch, turnCount, radius0,
            radius1, pointsPerTurn);
    }
    ...
    /// </param>
    /// <returns>NurbsCurve on success, null on failure.</returns>
    public static NurbsCurve CreateSpiral(Remote<Curve> railCurve, double t0, double t1, Point3d radiusPoint,
        double pitch, double turnCount, double radius0, double radius1, int pointsPerTurn)
    {
        return ComputeServer.Post<NurbsCurve>(ApiAddress(), railCurve, t0, t1, radiusPoint, pitch, turnCount, radius0,
            radius1, pointsPerTurn);
    }
...
public static class NurbsSurfaceCompute
{
    ...
    /// <param name="levels">
    ///     The number of levels (between 1 and 3) to be used in multi-level solver. Use 1 for single level
    ///     solve.
    /// </param>
    /// <returns>
    ...
    /// </remarks>
    public static Point2d[] CreateCurveOnSurfacePoints(Surface surface, IEnumerable<Point2d> fixedPoints,
        double tolerance, bool periodic, int initCount, int levels)
    {
        return ComputeServer.Post<Point2d[]>(ApiAddress(), surface, fixedPoints, tolerance, periodic, initCount,
            levels);
    }
    ...
    ///     To create a curve from the output points, use Surface.CreateCurveOnSurface.
    /// </remarks>
    public static Point2d[] CreateCurveOnSurfacePoints(Remote<Surface> surface, IEnumerable<Point2d> fixedPoints,
        double tolerance, bool periodic, int initCount, int levels)
    {
        return ComputeServer.Post<Point2d[]>(ApiAddress(), surface, fixedPoints, tolerance, periodic, initCount,
            levels);
    ...
    /// </remarks>
    public static NurbsCurve CreateCurveOnSurface(Surface surface, IEnumerable<Point2d> points, double tolerance,
        bool periodic)
    {
    ...
    ///     To produce the input points, use Surface.CreateCurveOnSurfacePoints.
    /// </remarks>
    public static NurbsCurve CreateCurveOnSurface(Remote<Surface> surface, IEnumerable<Point2d> points,
        double tolerance, bool periodic)
    {
        return ComputeServer.Post<NurbsCurve>(ApiAddress(), surface, points, tolerance, periodic);
    }
    ...
    public static bool MakeCompatible(Surface surface0, Surface surface1, out NurbsSurface nurb0,
        out NurbsSurface nurb1)
    {
        return ComputeServer.Post<bool, NurbsSurface, NurbsSurface>(ApiAddress(), out nurb0, out nurb1, surface0,
            surface1);
    }
    ...
    public static NurbsSurface CreateFromPoints(IEnumerable<Point3d> points, int uCount, int vCount, int uDegree,
        int vDegree)
    {
    ...
    /// <returns>A NurbsSurface on success or null on failure.</returns>
    /// <remarks>uCount multiplied by vCount must equal the number of points supplied.</remarks>
    public static NurbsSurface CreateThroughPoints(IEnumerable<Point3d> points, int uCount, int vCount, int uDegree,
        int vDegree, bool uClosed, bool vClosed)
    {
        return ComputeServer.Post<NurbsSurface>(ApiAddress(), points, uCount, vCount, uDegree, vDegree, uClosed,
            vClosed);
    }
    ...
    ///     <para>
    ///         This is the same as calling <see cref="CreateFromCorners(Point3d,Point3d,Point3d,Point3d,double)" /> with
    ///         tolerance 0.
    ///     </para>
    /// </summary>
    ...
    /// <param name="scaleHeight">If true, surface will be locally scaled.</param>
    /// <returns>A NurbsSurface or null on failure.</returns>
    public static NurbsSurface CreateRailRevolvedSurface(Remote<Curve> profile, Remote<Curve> rail, Line axis,
        bool scaleHeight)
    {
        return ComputeServer.Post<NurbsSurface>(ApiAddress(), profile, rail, axis, scaleHeight);
    }
    ...
    /// </param>
    /// <returns>A NurbsSurface or null on failure.</returns>
    public static NurbsSurface CreateNetworkSurface(IEnumerable<Curve> uCurves, int uContinuityStart,
        int uContinuityEnd, IEnumerable<Curve> vCurves, int vContinuityStart, int vContinuityEnd, double edgeTolerance,
        double interiorTolerance, double angleTolerance, out int error)
    {
        return ComputeServer.Post<NurbsSurface, int>(ApiAddress(), out error, uCurves, uContinuityStart, uContinuityEnd,
            vCurves, vContinuityStart, vContinuityEnd, edgeTolerance, interiorTolerance, angleTolerance);
    }
    ...
    /// </param>
    /// <returns>A NurbsSurface or null on failure.</returns>
    public static NurbsSurface CreateNetworkSurface(Remote<IEnumerable<Curve>> uCurves, int uContinuityStart,
        int uContinuityEnd, Remote<IEnumerable<Curve>> vCurves, int vContinuityStart, int vContinuityEnd,
        double edgeTolerance, double interiorTolerance, double angleTolerance, out int error)
    {
        return ComputeServer.Post<NurbsSurface, int>(ApiAddress(), out error, uCurves, uContinuityStart, uContinuityEnd,
            vCurves, vContinuityStart, vContinuityEnd, edgeTolerance, interiorTolerance, angleTolerance);
    }
    ...
    /// </param>
    /// <returns>A NurbsSurface or null on failure.</returns>
    public static NurbsSurface CreateNetworkSurface(IEnumerable<Curve> curves, int continuity, double edgeTolerance,
        double interiorTolerance, double angleTolerance, out int error)
    {
        return ComputeServer.Post<NurbsSurface, int>(ApiAddress(), out error, curves, continuity, edgeTolerance,
            interiorTolerance, angleTolerance);
    }
    ...
    /// </param>
    /// <returns>A NurbsSurface or null on failure.</returns>
    public static NurbsSurface CreateNetworkSurface(Remote<IEnumerable<Curve>> curves, int continuity,
        double edgeTolerance, double interiorTolerance, double angleTolerance, out int error)
    {
        return ComputeServer.Post<NurbsSurface, int>(ApiAddress(), out error, curves, continuity, edgeTolerance,
            interiorTolerance, angleTolerance);
...
public static class SurfaceCompute
{
    ...
    /// <returns>A new array of rolling ball fillet surfaces; this array can be empty on failure.</returns>
    /// <exception cref="ArgumentNullException">If surfaceA or surfaceB are null.</exception>
    public static Surface[] CreateRollingBallFillet(Remote<Surface> surfaceA, Remote<Surface> surfaceB, double radius,
        double tolerance)
    {
        return ComputeServer.Post<Surface[]>(ApiAddress(), surfaceA, surfaceB, radius, tolerance);
    }
    ...
    /// <returns>A new array of rolling ball fillet surfaces; this array can be empty on failure.</returns>
    /// <exception cref="ArgumentNullException">If surfaceA or surfaceB are null.</exception>
    public static Surface[] CreateRollingBallFillet(Surface surfaceA, bool flipA, Surface surfaceB, bool flipB,
    ...
    /// <returns>A new array of rolling ball fillet surfaces; this array can be empty on failure.</returns>
    /// <exception cref="ArgumentNullException">If surfaceA or surfaceB are null.</exception>
    public static Surface[] CreateRollingBallFillet(Remote<Surface> surfaceA, bool flipA, Remote<Surface> surfaceB,
        bool flipB, double radius, double tolerance)
    {
        return ComputeServer.Post<Surface[]>(ApiAddress(), surfaceA, flipA, surfaceB, flipB, radius, tolerance);
    }
    ...
    /// <returns>A new array of rolling ball fillet surfaces; this array can be empty on failure.</returns>
    /// <exception cref="ArgumentNullException">If surfaceA or surfaceB are null.</exception>
    public static Surface[] CreateRollingBallFillet(Surface surfaceA, Point2d uvA, Surface surfaceB, Point2d uvB,
        double radius, double tolerance)
    {
    ...
    /// <returns>A new array of rolling ball fillet surfaces; this array can be empty on failure.</returns>
    /// <exception cref="ArgumentNullException">If surfaceA or surfaceB are null.</exception>
    public static Surface[] CreateRollingBallFillet(Remote<Surface> surfaceA, Point2d uvA, Remote<Surface> surfaceB,
        Point2d uvB, double radius, double tolerance)
    {
        return ComputeServer.Post<Surface[]>(ApiAddress(), surfaceA, uvA, surfaceB, uvB, radius, tolerance);
    }
    ...
    public static Surface CreateSoftEditSurface(Surface surface, Point2d uv, Vector3d delta, double uLength,
        double vLength, double tolerance, bool fixEnds)
    {
    ...
    /// <param name="fixEnds">Keeps edge locations fixed.</param>
    /// <returns>The soft edited surface if successful. null on failure.</returns>
    public static Surface CreateSoftEditSurface(Remote<Surface> surface, Point2d uv, Vector3d delta, double uLength,
        double vLength, double tolerance, bool fixEnds)
    {
        return ComputeServer.Post<Surface>(ApiAddress(), surface, uv, delta, uLength, vLength, tolerance, fixEnds);
    }
    ...
    /// <summary>
    ///     Copies a surface so that all locations at the corners of the copied surface are specified distances from the
    ///     original surface.
    /// </summary>
    ...
    public static Surface VariableOffset(this Surface surface, out Surface updatedInstance, double uMinvMin,
        double uMinvMax, double uMaxvMin, double uMaxvMax, double tolerance)
    {
        return ComputeServer.Post<Surface, Surface>(ApiAddress(), out updatedInstance, surface, uMinvMin, uMinvMax,
            uMaxvMin, uMaxvMax, tolerance);
    }
    ...
    /// <param name="tolerance">The offset tolerance.</param>
    /// <returns>The offset surface if successful, null otherwise.</returns>
    public static Surface VariableOffset(Remote<Surface> surface, out Surface updatedInstance, double uMinvMin,
        double uMinvMax, double uMaxvMin, double uMaxvMax, double tolerance)
    {
        return ComputeServer.Post<Surface, Surface>(ApiAddress(), out updatedInstance, surface, uMinvMin, uMinvMax,
            uMaxvMin, uMaxvMax, tolerance);
    ...
    /// <summary>
    ///     Copies a surface so that all locations at the corners, and from specified interior locations, of the copied surface
    ///     are specified distances from the original surface.
    /// </summary>
    /// <param name="uMinvMin">Offset distance at Domain(0).Min, Domain(1).Min.</param>
    /// <param name="uMinvMax">Offset distance at Domain(0).Min, Domain(1).Max.</param>
    /// <param name="uMaxvMin">Offset distance at Domain(0).Max, Domain(1).Min.</param>
    ...
    /// <returns>The offset surface if successful, null otherwise.</returns>
    public static Surface VariableOffset(this Surface surface, out Surface updatedInstance, double uMinvMin,
        double uMinvMax, double uMaxvMin, double uMaxvMax, IEnumerable<Point2d> interiorParameters,
        IEnumerable<double> interiorDistances, double tolerance)
    {
        return ComputeServer.Post<Surface, Surface>(ApiAddress(), out updatedInstance, surface, uMinvMin, uMinvMax,
            uMaxvMin, uMaxvMax, interiorParameters, interiorDistances, tolerance);
    }
    ...
    /// </param>
    /// <returns>new rebuilt surface on success. null on failure.</returns>
    public static NurbsSurface Rebuild(Remote<Surface> surface, int uDegree, int vDegree, int uPointCount,
        int vPointCount)
    {
        return ComputeServer.Post<NurbsSurface>(ApiAddress(), surface, uDegree, vDegree, uPointCount, vPointCount);
    }
    ...
    /// <summary>
    ///     Rebuilds an existing surface with a new surface to a given point count in either the u or v directions
    ///     independently.
    /// </summary>
    ...
    /// <returns>new rebuilt surface on success. null on failure.</returns>
    public static NurbsSurface RebuildOneDirection(this Surface surface, int direction, int pointCount,
        LoftType loftType, double refitTolerance)
    {
    ...
    /// <param name="refitTolerance">The refit tolerance. When in doubt, use the document's model absolute tolerance.</param>
    /// <returns>new rebuilt surface on success. null on failure.</returns>
    public static NurbsSurface RebuildOneDirection(Remote<Surface> surface, int direction, int pointCount,
        LoftType loftType, double refitTolerance)
    {
        return ComputeServer.Post<NurbsSurface>(ApiAddress(), surface, direction, pointCount, loftType, refitTolerance);
    }
    ...
    /// <param name="v">V parameter of the surface that is closest to testPoint.</param>
    /// <returns>true if the search is successful, false if the search fails.</returns>
    public static bool LocalClosestPoint(this Surface surface, Point3d testPoint, double seedU, double seedV,
    ...
    /// <param name="v">V parameter of the surface that is closest to testPoint.</param>
    /// <returns>true if the search is successful, false if the search fails.</returns>
    public static bool LocalClosestPoint(Remote<Surface> surface, Point3d testPoint, double seedU, double seedV,
        out double u, out double v)
    {
        return ComputeServer.Post<bool, double, double>(ApiAddress(), out u, out v, surface, testPoint, seedU, seedV);
    }
    ...
    /// <param name="tolerance">
    ///     Tolerance used for the fit of the push-up curve. Generally, the resulting interpolating curve
    ///     will be within tolerance of the surface.
    /// </param>
    ...
    /// </param>
    /// <param name="closed">
    ///     If false, the interpolating curve is not closed. If true, the interpolating curve is closed, and
    ///     the last point and first point should generally not be equal.
    /// </param>
    ...
    ///     then if closedSurfaceHandling=1 a point(u, v) in points can have any value for the u coordinate, but must have 0
    ///     &lt;=v&lt;=1.
    ///     In particular, if points = { (0.0,0.5), (2.0,0.5) } then the interpolating curve will wrap around the surface two
    ///     times in the closed direction before ending at start of the curve.
    ///     If closed=true the last point should equal the first point plus an integer multiple of the period on a closed
    ///     direction.
    /// </param>
    /// <returns>A new NURBS curve if successful, or null on error.</returns>
    public static NurbsCurve InterpolatedCurveOnSurfaceUV(this Surface surface,
...
public static class GrasshopperCompute
{
    ...
    {
        [JsonProperty(PropertyName = "algo")] public string Algo { get; set; }
...
public static class IntersectionCompute
{
    ...
    /// <param name="plane">Plane to intersect with.</param>
    /// <returns>
    ///     An array of polylines describing the intersection loops or null (Nothing in Visual Basic) if no intersections
    ///     could be found.
    /// </returns>
    ...
    /// <returns>true on success, false on failure.</returns>
    public static bool BrepPlane(Brep brep, Plane plane, double tolerance, out Curve[] intersectionCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
    ...
    /// <param name="intersectionPoints">The intersection points will be returned here.</param>
    /// <returns>true on success, false on failure.</returns>
    public static bool BrepPlane(Remote<Brep> brep, Plane plane, double tolerance, out Curve[] intersectionCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
    ...
    /// <param name="tolerance">
    ///     Intersection tolerance. If the curves approach each other to within tolerance, an intersection
    ///     is assumed.
    /// </param>
    ...
    ///     <code source='examples\py\ex_intersectcurves.py' lang='py' />
    /// </example>
    public static CurveIntersections CurveCurve(Remote<Curve> curveA, Remote<Curve> curveB, double tolerance,
    ...
    /// <param name="overlapTolerance">The tolerance with which the curves are tested.</param>
    /// <returns>A collection of intersection events.</returns>
    public static CurveIntersections CurveLine(Remote<Curve> curve, Line line, double tolerance,
        double overlapTolerance)
    {
        return ComputeServer.Post<CurveIntersections>(ApiAddress(), curve, line, tolerance, overlapTolerance);
    }
    ...
    /// <param name="tolerance">
    ///     Intersection tolerance. If the curve approaches the surface to within tolerance, an
    ...
    /// </example>
    public static CurveIntersections CurveSurface(Curve curve, Surface surface, double tolerance,
        double overlapTolerance)
    {
    ...
    ///     <code source='examples\py\ex_curvesurfaceintersect.py' lang='py' />
    /// </example>
    public static CurveIntersections CurveSurface(Remote<Curve> curve, Remote<Surface> surface, double tolerance,
        double overlapTolerance)
    {
        return ComputeServer.Post<CurveIntersections>(ApiAddress(), curve, surface, tolerance, overlapTolerance);
    }
    ...
    /// <param name="overlapTolerance">The tolerance with which the curves are tested.</param>
    /// <returns>A collection of intersection events.</returns>
    public static CurveIntersections CurveSurface(Curve curve, Interval curveDomain, Surface surface, double tolerance,
        double overlapTolerance)
    {
        return ComputeServer.Post<CurveIntersections>(ApiAddress(), curve, curveDomain, surface, tolerance,
            overlapTolerance);
    }
    ...
    /// <param name="overlapTolerance">The tolerance with which the curves are tested.</param>
    /// <returns>A collection of intersection events.</returns>
    public static CurveIntersections CurveSurface(Remote<Curve> curve, Interval curveDomain, Remote<Surface> surface,
        double tolerance, double overlapTolerance)
    {
        return ComputeServer.Post<CurveIntersections>(ApiAddress(), curve, curveDomain, surface, tolerance,
            overlapTolerance);
    }
    ...
    /// </example>
    public static bool CurveBrep(Curve curve, Brep brep, double tolerance, out Curve[] overlapCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out overlapCurves, out intersectionPoints,
    ...
    ///     <code source='examples\py\ex_elevation.py' lang='py' />
    /// </example>
    public static bool CurveBrep(Remote<Curve> curve, Remote<Brep> brep, double tolerance, out Curve[] overlapCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out overlapCurves, out intersectionPoints,
            curve, brep, tolerance);
    ...
    /// <param name="t">Curve parameters at intersections.</param>
    /// <returns>True on success, false on failure.</returns>
    public static bool CurveBrep(Remote<Curve> curve, Remote<Brep> brep, double tolerance, double angleTolerance,
        out double[] t)
    {
        return ComputeServer.Post<bool, double[]>(ApiAddress(), out t, curve, brep, tolerance, angleTolerance);
    }
    ...
    /// <returns>true on success, false on failure.</returns>
    public static bool CurveBrepFace(Curve curve, BrepFace face, double tolerance, out Curve[] overlapCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out overlapCurves, out intersectionPoints,
    ...
    /// <param name="intersectionPoints">A points array argument. This out reference is assigned during the call.</param>
    /// <returns>true on success, false on failure.</returns>
    public static bool CurveBrepFace(Remote<Curve> curve, BrepFace face, double tolerance, out Curve[] overlapCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out overlapCurves, out intersectionPoints,
            curve, face, tolerance);
    ...
    /// <param name="intersectionPoints">The intersection points will be returned here.</param>
    /// <returns>true on success, false on failure.</returns>
    public static bool SurfaceSurface(Surface surfaceA, Surface surfaceB, double tolerance,
        out Curve[] intersectionCurves, out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
            surfaceA, surfaceB, tolerance);
    }
    ...
    /// <param name="intersectionPoints">The intersection points will be returned here.</param>
    /// <returns>true on success, false on failure.</returns>
    public static bool SurfaceSurface(Remote<Surface> surfaceA, Remote<Surface> surfaceB, double tolerance,
        out Curve[] intersectionCurves, out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
    ...
    /// <param name="intersectionPoints">The intersection points will be returned here.</param>
    /// <returns>true on success; false on failure.</returns>
    public static bool BrepBrep(Brep brepA, Brep brepB, double tolerance, out Curve[] intersectionCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
    ...
    /// <param name="intersectionPoints">The intersection points will be returned here.</param>
    /// <returns>true on success; false on failure.</returns>
    public static bool BrepBrep(Remote<Brep> brepA, Remote<Brep> brepB, double tolerance,
        out Curve[] intersectionCurves, out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
    ...
    /// <param name="tolerance">A tolerance value.</param>
    /// <param name="intersectionCurves">
    ///     The intersection curves array argument. This out reference is assigned during the
    ///     call.
    /// </param>
    /// <param name="intersectionPoints">
    ///     The intersection points array argument. This out reference is assigned during the
    ///     call.
    /// </param>
    /// <returns>true on success; false on failure.</returns>
    public static bool BrepSurface(Brep brep, Surface surface, double tolerance, out Curve[] intersectionCurves,
        out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
            brep, surface, tolerance);
    }
    ...
    /// </param>
    /// <returns>true on success; false on failure.</returns>
    public static bool BrepSurface(Remote<Brep> brep, Remote<Surface> surface, double tolerance,
        out Curve[] intersectionCurves, out Point3d[] intersectionPoints)
    {
        return ComputeServer.Post<bool, Curve[], Point3d[]>(ApiAddress(), out intersectionCurves,
            out intersectionPoints,
    ...
    /// </returns>
    public static Point3d[] ProjectPointsToMeshes(IEnumerable<Mesh> meshes, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance)
    {
    ...
    ///     Array of projected points, or null in case of any error or invalid input.
    /// </returns>
    public static Point3d[] ProjectPointsToMeshes(Remote<IEnumerable<Mesh>> meshes, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance)
    {
        return ComputeServer.Post<Point3d[]>(ApiAddress(), meshes, points, direction, tolerance);
    }
    ...
    /// </example>
    public static Point3d[] ProjectPointsToMeshesEx(IEnumerable<Mesh> meshes, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance, out int[] indices)
    {
    ...
    ///     <code source='examples\py\ex_projectpointstomeshesex.py' lang='py' />
    /// </example>
    public static Point3d[] ProjectPointsToMeshesEx(Remote<IEnumerable<Mesh>> meshes, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance, out int[] indices)
    {
        return ComputeServer.Post<Point3d[], int[]>(ApiAddress(), out indices, meshes, points, direction, tolerance);
    }
    ...
    /// </example>
    public static Point3d[] ProjectPointsToBreps(IEnumerable<Brep> breps, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance)
    {
    ...
    ///     <code source='examples\py\ex_projectpointstobreps.py' lang='py' />
    /// </example>
    public static Point3d[] ProjectPointsToBreps(Remote<IEnumerable<Brep>> breps, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance)
    {
        return ComputeServer.Post<Point3d[]>(ApiAddress(), breps, points, direction, tolerance);
    }
    ...
    ///     Array of projected points, or null in case of any error or invalid input.
    /// </returns>
    public static Point3d[] ProjectPointsToBrepsEx(IEnumerable<Brep> breps, IEnumerable<Point3d> points,
        Vector3d direction, double tolerance, out int[] indices)
    {
...
public class GrasshopperObject
{
    ...
    /// </summary>
    private class GeometryResolver : DefaultContractResolver
    {
            ...
            MemberSerialization memberSerialization)
        {
            ...
            if (property.DeclaringType == typeof(Point3f) ||
                ...
                property.ShouldSerialize = _ =>
                {
                    return property.PropertyName == "X" || property.PropertyName == "Y" ||
...
public class GrasshopperDataTree
{
    ...
    {
        get => ((IDictionary<string, List<GrasshopperObject>>)InnerTree)[key];

        set => ((IDictionary<string, List<GrasshopperObject>>)InnerTree)[key] = value;
    }

    public bool Contains(GrasshopperObject item)
    {
        foreach (var list in InnerTree.Values)
    ...
    {
        if (!InnerTree.ContainsKey(GhPath)) InnerTree.Add(GhPath, new List<GrasshopperObject>());

        InnerTree[GhPath].AddRange(items);
